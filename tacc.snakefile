...
rule perform_counting: 
  ...
  input: "bams/{sample}.GRCh38.ens77.tophat.sorted.bam", GTFFILE
  log: "log/{sample}.featureCounts.log"
  threads: THREADS
  message: "performing featureCounting with {threads} threads on genes in {input}"
  shell: "{FEATURECOUNTS}  -T {threads} --primary -F GTF -t exon -g gene_id -a {GTFFILE} -o counts/{wildcards.sample}.GRCh38.ens77.featureCounts.counts bams/{wildcards.sample}.GRCh38.ens77.tophat.sorted.bam 2> {log}"
...
rule qc_check: 
  ...
  shell: " perl {HASSL}/scripts/qc.pl --maplogfile log/{wildcards.sample}.tophat.log --metricsfile qc/{wildcards.sample}.GRCh38.ens77.tophat.crsm --sra {wildcards.sample} 2> {log}"

rule picard_rnaseq_qual: 
  output: "qc/{sample}.GRCh38.ens77.tophat.crsm"
  input: "bams/{sample}.GRCh38.ens77.tophat.sorted.bam", PICARDFLATFILE
...
rule index_bam: 
  output: temp("bams/{sample}.GRCh38.ens77.tophat.sorted.bam.bai")
  input: "bams/{sample}.GRCh38.ens77.tophat.sorted.bam"
  message: "indexing bam alignment file {input}"
  shell: "module load samtools;  {SAMTOOLS} index {input} {output} "

rule sort_bam:
  output: temp("bams/{sample}.GRCh38.ens77.hisat.sorted.bam")
  input: "bams/{sample}.GRCh38.ens77.tophat.bam"
  threads: THREADS
  message: "sorting {input} to {output}"
  shell: " module load samtools; {SAMTOOLS} sort -@ {threads} {input} bams/{wildcards.sample}.GRCh38.ens77.hisat.sorted "

rule sam_to_bam:
  output: temp("bams/{sample}.GRCh38.ens77.tophat.bam")
  input: "bams/{sample}.GRCh38.ens77.tophat.sam"
  message: "converting sam to bam: {input} to {output}"
  shell: " module load samtools; {SAMTOOLS} view -bS {input} > {output} "

rule tophat_alignment: 
  output: temp("bams/{sample}.GRCh38.ens77.tophat.sam")
  ...
  threads: THREADS
  log: "log/{sample}.tophat.log"
  message: "running tophat alignment on {wildcards.sample} with {threads} threads"
  shell: "export SHELL=/bin/zsh; echo $SHELL >> {log}.shell; module load bowtie/2.1.0; module load tophat/2.0.10; module load samtools; {TOPHAT} -p {threads} -G {GTFFILE} --resume --transcriptome-index {TOPHAT_TRANSCRIPT_INDEX} -z pigz  -o {output}  {HISATREF}  $SCRATCH/{wildcards.sample}.fastq   2> {log}"   
