...
rule all: 
	input: expand("{sample}.GRCh38.p4.HTSeq.counts", sample=SAMPLES), expand("{sample}.qc_check.done, sample=SAMPLES) 
...
rule perform_counting: 
	...
	shell: "time {HTSEQ} -m intersection-nonempty -i gene -s no -f bam {wildcards.sample}.GRCh38.p4.hisat.sorted.bam {GFFFILE} > {wildcards.sample}.GRCh38.p4.HTSeq.counts 2> {wildcards.sample}.HTseq-count.log"
...
rule picard_rnaseq_qual: 
	...
	shell: "time {PICARD} CollectRnaSeqMetrics REF_FLAT=~/refs/refflat/ncbirefflat.txt STRAND=NONE INPUT={wildcards.sample}.GRCh38.p4.hisat.sorted.bam OUTPUT={output} 2> {wildcards.sample}.picard_qual.log"
